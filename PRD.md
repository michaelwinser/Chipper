...
- **Deleting a Swimlane** requires a destination for anything inside it — another Swimlane, or **Archive**. There is no silent cascade. *(Revised at M5: this originally said "or The Pile". Piling a Goal destroys its Plans and Tasks, which contradicts §5.5's promise that the Pile is guilt-free and reversible — the Pile holds a line of text. Archive keeps everything and can be undone, and the restore rule below already anticipates a Goal whose Swimlane is gone. Unfinished loose Tasks still become Pile items, which loses only their size.)* *(Revised again at M8: **finished** loose Tasks are destroyed rather than piled. An idea has no notion of being done, so routing completed work through the Pile dropped `done`, `doneAt`, `notes` and `deadline` and put finished work back in the backlog as something still to do — which contradicts §5.5's own "done items belong to structure, not to the backlog". The confirmation states how many are destroyed before you agree.)*
